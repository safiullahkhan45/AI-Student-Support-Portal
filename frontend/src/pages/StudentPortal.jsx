    ...
    <div className="flex h-screen bg-slate-100 dark:bg-slate-950 overflow-hidden">
      ...
      <aside className={`fixed md:static inset-y-0 left-0 z-30 w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-700/60 flex flex-col transform transition-transform duration-200 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
        ...
        <div className="px-3 py-4 border-t border-slate-100 dark:border-slate-700/60">
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800">
            ...
            <button onClick={handleLogout} title="Logout" className="text-slate-400 hover:text-rose-500 dark:hover:text-rose-400 transition shrink-0">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 overflow-auto">
          {activeTab === 'chat'      && <div className="h-full p-4 sm:p-6"><ChatWindow /></div>}
          {activeTab === 'fees'      && <div className="p-4 sm:p-6"><FeeLookup /></div>}
          {activeTab === 'results'   && <div className="p-4 sm:p-6"><ResultLookup /></div>}
          {activeTab === 'complaint' && <div className="p-4 sm:p-6"><ComplaintSubmit /></div>}
          {activeTab === 'track'     && <div className="p-4 sm:p-6"><ComplaintTrack /></div>}
        </main>
      </div>
    </div>
  )
...
function FeeLookup() {
  ...
  async function handleSearch(e) {
    e.preventDefault(); setLoading(true); setError(null); setFeeRecord(null)
    try {
  ...
  const outstanding = feeRecord ? (parseFloat(feeRecord.amount_due) - parseFloat(feeRecord.amount_paid)).toFixed(2) : null
  const isPaid = feeRecord && parseFloat(outstanding) <= 0

  return (
    <div className="max-w-xl">
      <PageHeader title="Fee Status" subtitle="Check your current semester fee details." />
...
function ResultLookup() {
  ...
  const gpaColor = result
    ? result.gpa >= 3.5 ? 'text-emerald-600 dark:text-emerald-400' : result.gpa >= 2.5 ? 'text-blue-600 dark:text-blue-400' : 'text-amber-600 dark:text-amber-400'
    : ''

  return (
    <div className="max-w-2xl">
      <PageHeader title="My Results" subtitle="View your semester transcript and GPA." />
      ...
      {result && (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            {[
              { label: 'GPA', value: result.gpa.toFixed(2), sub: result.semester, cls: gpaColor, big: true },
      ...
      {complaints.length === 0 ? (
        ...
        <div className="space-y-3">
          {complaints.map((c) => {
            ...
            return (
              <div key={c.id} className={`bg-white dark:bg-slate-900 border rounded-2xl overflow-hidden shadow-sm transition ${isOpen ? 'border-indigo-300 dark:border-indigo-600' : 'border-slate-200 dark:border-slate-700/60'}`}>
